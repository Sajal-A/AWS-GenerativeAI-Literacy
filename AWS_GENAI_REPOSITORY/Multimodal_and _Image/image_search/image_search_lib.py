...
## Add the function to generate an embedding with the Amazon Titan Multimodal Embeddings model.
# This function will accept either an image or phrase to match.
...
## Add the function to get search results for the image or text query
...
def get_similarity_search_results(search_term = None, search_image=None):
    search_image_base4 = (get_base64_from_bytes(search_image) if search_image else None)

    query_embedding = get_multimodal_vector(input_text=search_term, input_image_base64=search_image_base4)
